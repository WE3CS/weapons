...
public void T_GetPlayerDataCallback(Database database, DBResultSet results, const char[] error, int userid) {
  ...
  if (IsValidClient(clientIndex)) {
    if (results == null) {
      ...
      if (GetClientAuthId(clientIndex, AuthId_Steam2, steamid, sizeof(steamid), true)) {
        char query[255];
        FormatEx(query, sizeof(query), "REPLACE INTO %sweapons_timestamps (steamid, last_seen) VALUES ('%s', %d)",
...
void UpdatePlayerData(int client, char[] updateFields) {
  ...
  if (GetClientAuthId(client, AuthId_Steam2, steamid, sizeof(steamid), true)) {
    char query[1024];
    FormatEx(query, sizeof(query), "UPDATE %sweapons SET %s WHERE steamid = '%s'", g_TablePrefix, updateFields,
             steamid);
    DataPack pack = new DataPack();
    pack.WriteString(query);
...
public void T_ReCreateMainTableCallback(Database database, DBResultSet results, const char[] error, bool mysql) {
  if (results == null) {
    ...
    index += FormatEx(
        migrateQuery[index], sizeof(migrateQuery) - index,
        "																	\
			...
			hkp2000_tag, glock, glock_float, glock_trak, glock_trak_count, glock_tag, elite, elite_float, elite_trak, ",
        g_TablePrefix);
    index += FormatEx(
        migrateQuery[index], sizeof(migrateQuery) - index,
        "																	\
    ...
    if (mysql) {
      index += FormatEx(seedColumnsQuery[index], sizeof(seedColumnsQuery) - index,
				...
				ALTER TABLE %sweapons																						\
					...
					ADD COLUMN mp9_seed int(10) NOT NULL DEFAULT '-1' AFTER mp9_tag, ",
                        g_TablePrefix);
      index += FormatEx(seedColumnsQuery[index], sizeof(seedColumnsQuery) - index,
                        "										\
      ...
      Format(renameQuery, sizeof(renameQuery), "ALTER TABLE %sweapons RENAME TO %sweapons_tmp", g_TablePrefix,
             g_TablePrefix);
...
void AddWeaponColumns(bool mysql, const char[] weapon, bool seedColumn = true) {
  ...
  Format(query, sizeof(query), "ALTER TABLE %sweapons ADD %s_trak_count int(10) NOT NULL DEFAULT '0'", g_TablePrefix,
         weapon);
  txn.AddQuery(query);
  Format(query, sizeof(query), "ALTER TABLE %sweapons ADD %s_tag varchar(128) NOT NULL DEFAULT ''", g_TablePrefix,
         weapon);
  txn.AddQuery(query);
  if (seedColumn) {
    Format(query, sizeof(query), "ALTER TABLE %sweapons ADD %s_seed int(10) NOT NULL DEFAULT '-1'", g_TablePrefix,
           weapon);
    txn.AddQuery(query);
  }
  ...
  if (results == null) {
    ...
    Format(insertQuery, sizeof(insertQuery), "	\
			INSERT INTO %sweapons_timestamps  		\
				SELECT steamid, %d FROM %sweapons",
           g_TablePrefix, GetTime(), g_TablePrefix);
...
void DeleteInactivePlayerData() {
  if (g_iGraceInactiveDays > 0) {
    ...
    FormatEx(
        query, sizeof(query),
        "DELETE FROM %sweapons WHERE steamid in (SELECT steamid FROM %sweapons_timestamps WHERE last_seen < %d - (%d * 86400))",
        g_TablePrefix, g_TablePrefix, now, g_iGraceInactiveDays);
    DataPack pack = new DataPack();
    pack.WriteCell(now);
    pack.WriteString(query);
...
public void T_DeleteInactivePlayerDataCallback(Database database, DBResultSet results, const char[] error, DataPack pack) {
  ...
  if (results == null) {
    ...
    if (now > 0) {
      char query[255];
      FormatEx(query, sizeof(query), "DELETE FROM %sweapons_timestamps WHERE last_seen < %d - (%d * 86400)",
               g_TablePrefix, now, g_iGraceInactiveDays);
      DataPack newPack = new DataPack();
      newPack.WriteCell(0);
      newPack.WriteString(query);
